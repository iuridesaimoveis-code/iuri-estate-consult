...
const pillars = [
  ...
  { icon: WalletCards, title: "Suporte financeiro", text: "Orientação nas etapas de financiamento, análise de crédito e condições de pagamento." },
  { icon: Scale, title: "Suporte jurídico", text: "Acompanhamento das questões jurídicas e contratuais envolvidas na negociação." },
...
const structure = [
  { icon: WalletCards, title: "Financeiro", text: "Orientação sobre financiamento e condições de pagamento." },
  { icon: Scale, title: "Jurídico", text: "Suporte nas questões jurídicas e contratuais." },
  { icon: FileCheck2, title: "Documental", text: "Orientação e acompanhamento da documentação necessária." },
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Iuri Luiz | Compra, Venda e Locação de Imóveis" },
      { name: "description", content: "Corretor de imóveis para compra, venda e locação, com orientação financeira, jurídica e documental em toda a negociação." },
...
function Brand({ inverse = false }: { inverse?: boolean }) {
  return (
    <a href="#inicio" className={`flex items-center gap-3 ${inverse ? "text-hero-foreground" : "text-foreground"}`} aria-label="Iuri Luiz — início">
      <span className="grid size-10 place-items-center border border-current font-display text-lg">IL</span>
  ...
  return (
    <main className="overflow-hidden bg-background">
      <section id="inicio" className="relative min-h-[760px] bg-hero text-hero-foreground lg:min-h-[820px]">
        ...
        <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between border-b border-hero-border px-5 py-5 sm:px-8 lg:px-12">
          ...
          <nav aria-label="Navegação principal" className="hidden items-center gap-8 text-xs uppercase lg:flex">
            ...
            <a className="transition-colors hover:text-accent" href="#diferencial">Diferencial</a>
        ...
        <div className="relative z-10 mx-auto flex min-h-[650px] max-w-7xl items-end px-5 pb-14 pt-24 sm:px-8 lg:min-h-[700px] lg:items-center lg:px-12 lg:pb-20 lg:pt-28">
          <div className="max-w-4xl animate-reveal">
            ...
            <p className="mt-7 border-l border-accent pl-4 text-xs leading-5 text-hero-muted">Venda • Locação • Suporte financeiro • Suporte jurídico • Acompanhamento completo</p>
          </div>
        </div>
      ...
      <section id="atuacao" className="section-space">
        <div className="content-wrap">
          <div className="grid gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:gap-24">
            ...
            <p className="section-lead lg:pt-10">Uma negociação imobiliária envolve muito mais do que encontrar um imóvel. Existem valores, documentação, financiamento, contratos, análise e negociação. Meu trabalho é acompanhar cada etapa e conectar o cliente aos profissionais necessários para que o processo aconteça com mais segurança e clareza.</p>
          </div>
          <div className="mt-16 grid border-l border-t border-border sm:grid-cols-2 lg:grid-cols-4">
            {pillars.map(({ icon: Icon, title, text }, index) => <article key={title} className="group min-h-72 border-b border-r border-border p-7 transition-colors hover:bg-secondary lg:p-8"><span className="mb-14 flex items-center justify-between text-accent"><Icon className="size-6" strokeWidth={1.4} /><span className="text-xs text-muted-foreground">0{index + 1}</span></span><h3 className="font-display text-2xl">{title}</h3><p className="mt-4 text-sm leading-6 text-muted-foreground">{text}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="section-space border-y border-border bg-secondary">
        <div className="content-wrap">
          <div className="grid gap-8 lg:grid-cols-12"><div className="lg:col-span-4"><p className="eyebrow">O que eu faço</p><h2 className="section-title">Três caminhos.<br />Uma condução próxima.</h2></div><p className="section-lead lg:col-span-5 lg:col-start-8 lg:pt-10">Atendimento a compradores, proprietários e locatários, sempre com estratégia e clareza em cada decisão.</p></div>
      ...
      <section id="diferencial" className="section-space bg-primary text-primary-foreground">
        <div className="content-wrap relative">
          <span aria-hidden="true" className="pointer-events-none absolute -right-8 -top-28 hidden select-none font-display text-[18rem] text-primary-foreground/[0.025] lg:block">IL</span>
          <div className="relative grid gap-12 lg:grid-cols-12"><div className="lg:col-span-7"><p className="eyebrow text-accent">Estrutura para negociar</p><h2 className="section-title max-w-3xl">Um atendimento.<br /><em className="font-normal text-accent">Todas as etapas.</em></h2></div><p className="max-w-md self-end border-l border-primary-border pl-6 leading-7 text-primary-muted lg:col-span-4 lg:col-start-9">Da escolha do imóvel à documentação. Da negociação ao fechamento. O cliente conta com orientação e acompanhamento durante todo o processo.</p></div>
          <div className="relative mt-20 grid border-t border-primary-border sm:grid-cols-2 lg:grid-cols-4">{structure.map(({ icon: Icon, title, text }, index) => <article key={title} className={`py-9 lg:min-h-64 lg:px-8 ${index ? "border-t border-primary-border sm:border-l sm:border-t-0" : ""}`}><Icon className="size-5 text-accent" strokeWidth={1.4} /><h3 className="mt-14 font-display text-2xl">{title}</h3><p className="mt-4 text-sm leading-6 text-primary-muted">{text}</p></article>)}</div>
        </div>
      </section>
      ...
      <section id="sobre" className="section-space">
        <div className="content-wrap grid items-center gap-14 lg:grid-cols-12 lg:gap-8">
          <div className="relative lg:col-span-5 lg:col-start-2">
            <div className="aspect-[4/5] overflow-hidden bg-secondary"><img src={portraitAsset.url} alt="Iuri Luiz, corretor de imóveis" width={800} height={800} loading="lazy" className="size-full object-cover object-top grayscale-[18%] contrast-[1.04]" /></div>
          ...
          <div className="lg:col-span-5 lg:col-start-8"><p className="eyebrow">Sobre Iuri</p><h2 className="mt-5 font-display text-5xl sm:text-6xl">Iuri Luiz</h2><p className="mt-3 text-xs font-semibold uppercase text-accent">Corretor de Imóveis | CRECI 184010</p><blockquote className="mt-10 border-l border-border pl-7 font-display text-2xl leading-relaxed text-muted-foreground sm:text-3xl">“Meu trabalho é tornar o processo imobiliário mais simples, seguro e transparente, oferecendo acompanhamento próximo desde o primeiro contato até a conclusão da negociação.”</blockquote><ActionLink className="mt-10" href={whatsappUrl("Olá, Iuri! Gostaria de conversar sobre meu próximo negócio imobiliário.")} target="_blank" rel="noreferrer">Conversar com Iuri</ActionLink></div>
        </div>
      </section>

      <section className="relative min-h-[460px] overflow-hidden bg-hero text-hero-foreground">
        <img src={interiorImage} alt="Interior de arquitetura contemporânea" width={1024} height={1280} loading="lazy" className="absolute inset-0 size-full object-cover opacity-25" />
        <div className="absolute inset-0 bg-hero/80" />
        <div className="content-wrap relative z-10 flex min-h-[460px] items-center py-20"><div className="max-w-3xl"><p className="eyebrow text-accent">Atendimento imobiliário</p><p className="mt-6 font-display text-[clamp(2.5rem,5vw,4.8rem)] leading-[1.05]">Compra, venda e locação com estratégia, clareza e proximidade.</p></div></div>
      </section>
